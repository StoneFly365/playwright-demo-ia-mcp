...
|---|---|---|---|---|---|---|
| **P1** | **Ejecutar** la suite del proyecto filtrando por fichero, por navegador y por tag, e **interpretar** el informe distinguiendo un fallo de locator, uno de aserción y uno de la aplicación | 1 FOLLOW | Tabla comparativa de 6 formas de ejecución, con número de tests, tiempo y resultado, más la respuesta escrita sobre los 10 fallos | [§1, §2](theory.md#1-ejecutar-la-suite) | Lab 1 | Parte A (**A1, A7, A8**) + validación de E1 |
...
| **P6** | **Seleccionar** la aserción adecuada entre `toHaveText`, `toContainText`, `toHaveCount`, `toHaveURL` y `toBeVisible`, y redactar su mensaje descriptivo en lenguaje de negocio | 3 CREATE | Aserciones con mensaje en castellano, comprensibles sin abrir el código | [§4](theory.md#4-aserciones-web-first-y-las-otras) | Lab 4, Challenge | Parte A (**A6**) + E1 |
| **P7** | **Diagnosticar** un fallo causado por un locator ambiguo, por un locator que no encuentra nada o por un método sin reintento, y corregir la causa **sin tocar la aserción** | 5 TROUBLESHOOT | Informe con síntoma / causa raíz / corrección / cómo evitarlo, escrito **antes** de corregir | [§10, §11, §12](theory.md#10-strict-mode-la-ambigüedad-no-se-tapa) | **Lab 5** | Parte A (**A8**, compartido con P1) + **E2** |
...
3. **[Challenge 1](challenges/challenge-1-compra-completa.md) entregado**, cumpliendo AC1-AC6, con su `decisiones.md`. **Es el único instrumento de P8**: sin él, ese objetivo se queda sin evidencia.
4. ≥ 70 en las partes A y B del [assessment](assessment/).
5. **Apto** en la Parte C (defensa técnica), que se hace **sobre el `decisiones.md` del Challenge**.

## Matriz de trazabilidad

> **Fuente canónica de la asignación ítem → objetivo:** [`assessment/README.md`](assessment/), tabla *Trazabilidad objetivo ↔ ítem ↔ dónde se enseñó*. Es la que tiene delante el formador al puntuar y la única que mapea cada ítem a su sección de teoría y a su Lab. Esta matriz la reproduce; si alguna vez discrepan, manda la del assessment.

| Objetivo | Teoría | Anclaje en el repositorio | Lab | Challenge | Assessment |
|---|---|---|---|---|---|
| **P1** | §1, §2 | R2, R4, R5, R6, R28, R29 | Lab 1 | uso implícito | **A1, A7, A8**, validación de E1 |
...
| **P6** | §4 | R20, R21, R22, R23, R24 | Lab 4 | **AC2** | **A6**, E1 |
| **P7** | §10, §11, §12 | R8, R13b, R16, R16b | **Lab 5** | — | **A8** (compartido con P1), **E2** |
| **P8** | todo | R14, R18, R19, R20 | preparado por Labs 3-4 | **Challenge — entrega obligatoria (criterio de superación 3)** | **Parte C, defendida sobre el `decisiones.md` del Challenge** |
...
| ¿Algún objetivo se practica pero no se evalúa? | **No.** Los 8 aparecen en assessment, Challenge o Parte C. **P8 depende del Challenge, y por eso el Challenge es criterio de superación obligatorio (punto 3) y el objeto de la Parte C** |
